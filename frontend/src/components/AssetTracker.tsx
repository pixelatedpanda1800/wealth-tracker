...
export const AssetTracker: React.FC = () => {
  ...
  const latestIndex =
    data.length > 0
      ? data
          .slice()
          .reverse()
          .findIndex((d) => !d.isEstimate && (d.total ?? 0) > 0)
      : -1;
  ...
  const previous =
    latestIndex !== -1 && data.length - 2 - latestIndex >= 0
...
const StatCard: React.FC<StatCardProps> = ({ title, value, trend, icon, color }) => {
  ...
  return (
    <div
      className={clsx(
        'flex items-start justify-between rounded-2xl border p-6 shadow-sm transition-all',
        activeStyle,
      )}
    >
      <div className="space-y-1">
        ...
        >
          {trend} <span className="font-normal text-slate-400">vs last month</span>
        </p>
      </div>
