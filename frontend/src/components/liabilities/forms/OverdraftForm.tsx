...
export const OverdraftForm: React.FC<FormProps> = ({ form, setField, setMeta }) => (
  <div className="space-y-4">
    <div className="grid grid-cols-2 gap-3">
      ...
      <Field label="Arranged Limit (£)" hint="The facility size, not the balance owed">
        <input
          ...
          value={form.typeMetadata.arrangedLimit ?? ''}
          onChange={(e) =>
            setMeta('arrangedLimit', e.target.value !== '' ? Number(e.target.value) : undefined)
          }
          className={inputCls}
        />
      </Field>
    ...
    <p className="rounded-xl border border-slate-100 bg-slate-50 p-3 text-xs text-slate-500">
      Overdraft balances are entered as the amount currently overdrawn (a positive number). The
      burndown projects clearance based on APR and any overpayments you set.
    </p>
  </div>
);
