  ...
  <div className="space-y-4">
    <Field label="Lender / Counterparty">
      <input
        ...
        value={form.typeMetadata.counterparty ?? ''}
        onChange={(e) => setMeta('counterparty', e.target.value || undefined)}
        className={inputCls}
      />
    </Field>
    ...
    <Field label="Agreed Repayment Schedule" hint="Free text — e.g. 'When I can afford it'">
      <input
        type="text"
        placeholder="e.g. £200/month from Jan 2026"
        value={form.typeMetadata.agreedSchedule ?? ''}
        onChange={(e) => setMeta('agreedSchedule', e.target.value || undefined)}
        className={inputCls}
      />
    </Field>
