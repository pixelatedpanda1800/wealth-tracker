  ...
  <div className="space-y-4">
    <Field label="Provider" hint="e.g. Klarna, Clearpay, Laybuy">
      <input
        ...
        value={form.typeMetadata.provider ?? ''}
        onChange={(e) => setMeta('provider', e.target.value || undefined)}
        className={inputCls}
      />
    </Field>
    ...
    <div className="grid grid-cols-2 gap-3">
      <Field label="Total Instalments" required>
        <input
          ...
          min={1}
          placeholder="e.g. 3"
          value={form.typeMetadata.instalmentCount ?? ''}
          onChange={(e) =>
            setMeta('instalmentCount', e.target.value !== '' ? Number(e.target.value) : undefined)
          }
          className={inputCls}
        />
      </Field>
      <Field label="Instalments Paid">
        <input
          ...
          min={0}
          placeholder="e.g. 1"
          value={form.typeMetadata.instalmentsPaid ?? ''}
          onChange={(e) =>
            setMeta('instalmentsPaid', e.target.value !== '' ? Number(e.target.value) : undefined)
          }
          className={inputCls}
        />
      </Field>
