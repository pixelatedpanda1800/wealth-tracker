  ...
  return (
    <div className="space-y-4">
      <Field label="Plan Type" required>
        <select
          value={planType}
          onChange={(e) => setMeta('planType', e.target.value)}
          className={selectCls}
        >
      ...
      <div className="grid grid-cols-2 gap-3">
        <Field label="Write-Off Year" hint="Year the debt is wiped">
          <input
            ...
            value={form.typeMetadata.writeOffYear ?? ''}
            onChange={(e) =>
              setMeta('writeOffYear', e.target.value !== '' ? Number(e.target.value) : undefined)
            }
            className={inputCls}
          />
        </Field>
        <Field label="Salary Threshold (£)" hint="Your plan's repayment threshold">
          <input
            type="number"
            min={0}
            placeholder="e.g. 27295"
            value={form.typeMetadata.salaryThreshold ?? ''}
            onChange={(e) =>
              setMeta('salaryThreshold', e.target.value !== '' ? Number(e.target.value) : undefined)
            }
            className={inputCls}
          />
        </Field>
      </div>
      <p className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs text-amber-600">
        UK student loan repayment depends on income, future threshold uplifts, and plan rules. The
        burndown chart shows the balance flat with a note — the write-off year is shown as the end
        point.
      </p>
    </div>
  );
