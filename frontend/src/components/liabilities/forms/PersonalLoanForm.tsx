        ...
        <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
                <Field label="Interest Rate (% APR)" required>
                    <input type="number" step="0.1" min={0} placeholder="e.g. 7.9" value={form.interestRate} onChange={e => setField('interestRate', e.target.value)} className={inputCls} />
                </Field>
                <Field label="Original Amount (£)" required>
                    <input type="number" min={0} placeholder="e.g. 10000" value={form.originalPrincipal} onChange={e => setField('originalPrincipal', e.target.value)} className={inputCls} />
                </Field>
            </div>
            <div className="grid grid-cols-2 gap-3">
                <Field label="Term (months)" required>
                    <input type="number" min={1} placeholder="e.g. 60" value={form.termMonths} onChange={e => setField('termMonths', e.target.value)} className={inputCls} />
                </Field>
                <Field label="Monthly Payment (£)" hint="Auto-calculated if left blank">
                    <input type="number" min={0} placeholder="e.g. 195" value={form.monthlyPayment} onChange={e => setField('monthlyPayment', e.target.value)} className={inputCls} />
                </Field>
            </div>
