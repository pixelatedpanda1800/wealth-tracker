  ...
  return (
    <div className="space-y-4">
      <Field label="Finance Type" required>
        <select
          value={subType}
          onChange={(e) => setMeta('subType', e.target.value)}
          className={selectCls}
        >
        ...
        <div className="grid grid-cols-2 gap-3">
          <Field label="Balloon / Final Payment (£)" hint="Amount due at end of agreement">
            <input
              ...
              value={form.typeMetadata.balloonPayment ?? ''}
              onChange={(e) =>
                setMeta(
                  'balloonPayment',
          ...
          <Field label="GMFV (£)" hint="Guaranteed Minimum Future Value">
            <input
              ...
              value={form.typeMetadata.gmfv ?? ''}
              onChange={(e) =>
                setMeta('gmfv', e.target.value !== '' ? Number(e.target.value) : undefined)
              }
              className={inputCls}
            />
          </Field>
      ...
      {subType === 'pcp' && (
        <Field label="Annual Mileage Cap">
          <input
            ...
            value={form.typeMetadata.mileageCap ?? ''}
            onChange={(e) =>
              setMeta('mileageCap', e.target.value !== '' ? Number(e.target.value) : undefined)
            }
            className={inputCls}
          />
        </Field>
