...
const periodCutoff = (period: Period): number => {
    ...
    switch (period) {
        case '1M': return nowIndex - 1;
        case '3M': return nowIndex - 3;
        case '1Y': return nowIndex - 12;
        case '5Y': return nowIndex - 60;
        case 'All': return 0;
    }
};
...
export const calculateHoldingStats = (
    ...
    const previousValue = previous ? Number(previous.value) : currentValue;

    const periodStart = periodSnaps[0];
    const periodStartValue = periodStart ? Number(periodStart.value) : currentValue;
    ...
    return { currentValue, previousValue, change, changePct, periodStartValue, costBasis, gainLoss, returnPct };
};
...
export const buildAccountChartData = (
    ...
    return periods.map(({ year, month }) => {
        ...
        for (const holding of accountHoldings) {
            ...
            point[`holding_${holding.id}`] = snap ? Number(snap.value) : 0;
        }
        return point;
    });
};
